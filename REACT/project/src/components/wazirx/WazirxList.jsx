...
import {WazirxItem} from"./WazirxItem";
import axios from "axios";

export const WazirxList=()=>{
    ...
    const getData= async ()=>{
        const result= await axios.get("https://api.wazirx.com/sapi/v1/tickers/24hr");
        setData(result.data)
        setFiltData(result.data.slice(0,50))
        
        const resultCat= await axios.get("https://api.wazirx.com/sapi/v1/tickers/24hr")
        setCatData(resultCat.data.slice(0,50));
    }
    useEffect(()=>{
        const filtred=data.filter((item)=>
        item.symbol.toUpperCase().includes(txt.toUpperCase())|| 
        item.lowPrice.toUpperCase().includes(txt.toUpperCase()))
